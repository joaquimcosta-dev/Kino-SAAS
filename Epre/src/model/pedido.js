...
export const criar_pedido= async(data)=>{
    const {nome,telefone,endereco,cod,datas}=data;
    const novo = await db.query("insert into pedido(nome,telefone,endereco,codigo,data) values (?,?,?,?,?)",[nome,telefone,endereco,cod,datas]);
    return novo;
}
